...
class Menu:
    def __init__(self, window):
        self.window = window
        self.surf = pygame.image.load("./asset/MenuBg.png").convert_alpha() # Loading the menu background image
    ...
    def run(self, ):
        menu_option = 0
        pygame.mixer_music.load('./asset/Menu.mp3')  # Chamando a música
        pygame.mixer_music.play(-1)  # reproduzindo a musica em loop

        while True:
            # DRAW IMAGES
            self.window.blit(source=self.surf, dest=self.rect)# "blit" will draw the image (Ensuring it appears within the rectangle)
            self.menu_text(50, "Star", COLOR_ORANGE, ((WIN_WIDTH / 2), 70))
